...
As an example, we will consider the 4-state sick-sicker model that has been described in more detail by [Alarid-Escudero et al](https://arxiv.org/abs/2001.07824). The model will be used to compare two treatment strategies, a "New" treatment and the existing "standard of care (SOC)". The sick- fare  consists 4 health states. Ordered from worst to best to worst, they are: Healthy (*H*), Sick (*S1*), Sicker (*S2*), and Death (*D*). Possible transitions from each state are displayed in the figure below.
...
At time $0$, the state vector, $x_0 = (1, 0, 0, 0)$ since all patients are healthy. The state vector at time $t+1$ for each of $T$ model cycles can be computed using matrix multiplication,
...
## Costs and outcome
To perform cost-effectiveness analysis (CEA), Markov models estimate expected (i.e., average) costs and outcomes for the cohort of interest. In our examples, quality-adjusted life-years (QALYs) will be used as the outcome. 
...
In health economics, future cost and outcomes are discounted using a discount rate. In discrete time, the "discount weight" at time $t$ is given by $1/(1 + r)^t$ where $r$ is the discount rate; in continuous time, it is given by $e^{-rt}$. In this example we will use discrete time, meaning that the present value of expected values are given by,
...
It is important to note that in mathematical terms, this a discrete approximation to a continuous integral, also known as a Riemann sum. This means that we only know that an expected value lies within a time interval (e.g., in model cycle 1, between year 0 and 1; in model cycle 2, between year 1 and 2; and so on). If we assume that events occur at the start of model cycles (i.e., a left Riemann sum), then expected values will be underestimated; conversely, if we assume that they occur at the end of model cycles (i.e., a right Riemann sum), then they will be overestimated. Another option is to use average of values at the left and right endpoints (i.e., the trapezoid rule). In this example, we will assume events occur at the start of model cycles.

# Model settings
Our example model will be simulated for a cohort of 25-year old individuals until a maximum age of 110; that is, for 85 years.

```{r, message = FALSE}
n_cycles <- 85
```

It is helpful to first initialize an empty cohort trace matrix where each row is a state vector and fill in the first row so that it contains the proportion of individuals that are initially in each of the four health states.

```{r, message = FALSE}
state_names <- c("H", "S1", "S2", "D")
n_states <- length(state_names)
x_soc <- matrix(NA, nrow = n_cycles + 1, ncol = n_states)
colnames(x_soc) <- state_names
x_soc[1, ] <- c(1, 0, 0, 0)
head(x_soc)
```

Discount rates are set for quality-adjusted life-years and costs.
...
## Transition probabilities
### Standard of care
The annual transition probabilities for SOC are defined as follows.
...
The relative risk can then be used to create the transition matrix for the new treatment. 

```{r, message = FALSE}
p_new <- p_soc

p_new["H", "S1"] <- p_new["H", "S1"] * rr
p_new["H", "S2"] <- p_new["H", "S2"] * rr
p_new["H", "D"] <- p_new["H", "S2"] * rr
p_new["H", "H"] <- 1 - sum(p_new["H", -1])

p_new["S1", "S2"] <- p_new["S1", "S2"] * rr
p_new["S1", "D"] <- p_new["S1", "D"] * rr
p_new["S1", "S1"] <- 1 - sum(p_new["S1", -2])

p_new["S2", "D"] <- p_new["S2", "D"] * rr
p_new["S2", "S2"] <- 1 - sum(p_new["S2", -3])
...
A for loop is used to compute the state vector at each model cycle. We perform the computation for both the SOC and new treatment. As expected, patients remain in less severe health states longer when using the new treatment.

```{r, message = FALSE}
x_new <- x_soc
for (t in 1:n_cycles){
  x_soc[t + 1, ] <- x_soc[t, ] %*% p_soc
  x_new[t + 1, ] <- x_new[t, ] %*% p_new
}
head(x_soc)
head(x_new)
```
...
To compute discounted costs and QALYS, it is helpful to write a simple general function for computing a present value.
...
dqalys_soc <- pv(x_soc %*% utility, dr_qalys, 1:(n_cycles + 1))
dqalys_new <- pv(x_new %*% utility, dr_qalys, 1:(n_cycles + 1))
...
```{r}
dcosts_soc <- cbind(
  pv(x_soc %*% costs_medical, dr_costs, 1:(n_cycles + 1)),
  pv(x_soc %*% costs_treat_soc, dr_costs, 1:(n_cycles + 1))
)

dcosts_new <- cbind(
  pv(x_new %*% costs_medical, dr_costs, 1:(n_cycles + 1)),
  pv(x_new %*% costs_treat_new, dr_costs, 1:(n_cycles + 1))
)
...
We conclude by computing the incremental cost-effectiveness ratio (ICER) with the new treatment relative to SOC.

```{r, message=FALSE}
# Sum costs and QALYs over model cycles
total_dcosts_soc <- sum(dcosts_soc)
total_dcosts_new <- sum(dcosts_new)
total_dqalys_soc <- sum(dqalys_soc)
total_dqalys_new <- sum(dqalys_new)

# Compute ICER
incr_total_dcosts <- total_dcosts_new - total_dcosts_soc
incr_total_dqalys <- total_dqalys_new - total_dqalys_soc

icer <- incr_total_dcosts/incr_total_dqalys
```

For presentation, we will make a nice looking table.

```{r}
format_costs <- function(x) formatC(x, format = "d", big.mark = ",")
format_qalys <- function(x) formatC(x, format = "f", digits = 2)
tibble(
  `Costs` = c(total_dcosts_soc, total_dcosts_new) %>%
    format_costs(), 
  `QALYs` = c(total_dqalys_soc, total_dqalys_new) %>%
    format_qalys(),
  `Inremental costs` = c("--", incr_total_dcosts %>% format_costs()),
  `Inremental QALYs` = c("--", incr_total_dqalys %>% format_qalys),
