...
# Required R-package 
Install and load R Package ‘expm’ to compute the k-th power of a matrix
...
```
State occupancy with monotherapy at 5 years (efficient)
```{r, message=FALSE}
...
```{r, message=FALSE}
follow_up <- 20
trace_mono<-matrix(rep(NA, 6 * follow_up), nrow=follow_up)  #create empty trace matrix for 6 variables and 20 cycles
colnames(trace_mono) <- c("State A", "State B", "State C", "Death", "Disc LY", "Disc cost") #create column headings
