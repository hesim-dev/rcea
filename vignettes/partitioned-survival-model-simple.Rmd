...
pkgdown:
  as_is: yes
vignette: |
  %\VignetteIndexEntry{Partitioned Survival Model} %\VignetteEngine{knitr::rmarkdown} \usepackage[utf8]{inputenc}
---
...
We begin by defining the population, treatment strategies, and model structure. In this example, we model three treatment strategies and four health states (death and three non-death states). This information is combined using `hesim_data()`, which creates a general object of class `hesim_data`.
...
Survival curves are predicted by treatment strategy with the function `$sim_survival()`. The predicted curves are returned as a tidy `data.table` to facilitate plotting and stored as a field in the class named `survival_`. Below we plot the mean survival curve for treatment strategy 3. 
