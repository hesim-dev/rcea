...
patients_dt <- data.table(patient_id = seq(1, 3),
                          age = c(45, 50, 60),
                          female = c(0, 0, 1))
patients_dt <- patients_dt[,grp_id :=  patient_id]
...
Survival curves are predicted by treatment strategy and patient with the function `$sim_survival()`. The predicted curves are returned as a tidy `data.table` to facilitate plotting and stored as a field in the class named `survival_`. Below we plot the mean survival curve for group 2 with treatment strategy 3. 
...
surv_means <- psm$survival_[, .(mean_surv = mean(survival)),
                                      by = c("curve", "strategy_id", "grp_id", "t")]
theme_set(theme_bw())
p_data <- surv_means[strategy_id == 3 & grp_id == 2]
...
```{r}
ce_sim <- psm$summarize(by_grp =TRUE)
