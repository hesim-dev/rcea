...
### Costs and QALYs
Costs and QALYs are computed by integrating the previously simulated state probabilities. To maintain consistency with our prior analyses, we measure costs and QALYs at the start of each model cycle using a left Riemann sum.
...
Although `icea_pw()` allows users to summarize output from a PSA we will just create an ICER table using means for now. A complete analysis is provided in the [CEA tutorial](06-cea.html).
